...
- 908aec2: refactor: move the code shared with `@react-native-healthkit/health-records` into the new `@react-native-healthkit/core` package.
...
- 0d5214e: Make typed `metadata` the canonical metadata API and generate more of the HealthKit type surface from Apple’s SDK.
...
- 390f784: feat: register observer queries in AppDelegate for background delivery
