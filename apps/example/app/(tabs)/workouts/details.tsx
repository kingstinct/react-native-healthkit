    ...
    return (
        <List scrollEnabled>
            ...
            <Section>
                <ListItem
                    title='Workout Type'
                    subtitle={workoutActivityTypeStrings[workout.sample.workoutActivityType]} />
                <ListItem
                    title='Started'
                ...
                { workout.sample.totalDistance ? <ListItem
                    title='Total Distance'
                    subtitle={`${Math.round(workout.sample.totalDistance.quantity)} ${workout.sample.totalDistance.unit}`} /> : null }
                { workout.sample.totalEnergyBurned ? <ListItem
                    title='Total Energy Burned'
                    subtitle={`${Math.round(workout.sample.totalEnergyBurned.quantity)} ${workout.sample.totalEnergyBurned.unit}`} /> : null }
                { workout.sample.totalFlightsClimbed ? <ListItem
                    title='Total Flights Climbed'
                    subtitle={`${Math.round(workout.sample.totalFlightsClimbed.quantity)} ${workout.sample.totalFlightsClimbed.unit}`} /> : null }
                { workout.sample.totalSwimmingStrokeCount ? <ListItem
                    title='Total Swimming Stroke Count'
                    subtitle={`${Math.round(workout.sample.totalSwimmingStrokeCount.quantity)} ${workout.sample.totalSwimmingStrokeCount.unit}`} /> : null }

            </Section>
            { workout.sample.device ? <Section title='Device'>
                { workout.sample.device.name ? <ListItem
                    title='Name'
                    subtitle={workout.sample.device.name} /> : null }
                { workout.sample.device.model ? <ListItem
                    title='Model'
                    subtitle={workout.sample.device.model} /> : null }
                { workout.sample.device.manufacturer ? <ListItem
                    title='Manufacturer'
                    subtitle={workout.sample.device.manufacturer} /> : null }
                { workout.sample.device.hardwareVersion ? <ListItem
                    title='Hardware Version'
                    subtitle={workout.sample.device.hardwareVersion} /> : null }
                { workout.sample.device.softwareVersion ? <ListItem
                    title='Software Version'
                    subtitle={workout.sample.device.softwareVersion} /> : null }
            </Section> : null}

            { workout.sample.metadata && Object.keys(workout.sample.metadata).length > 0 && <Section title='Metadata'>
            ...
            {
                workout.sample.events && workout.sample.events.length > 0 && <Section title='Events'>
            ...
            {
                workout.sample.activities && workout.sample.activities.length > 0 && <Section title='Activities'>
            ...
            {
                routes && routes.length > 0 && <Section title='Routes'>
