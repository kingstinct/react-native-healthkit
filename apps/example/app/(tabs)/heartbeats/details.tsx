...
export default function HeartbeatSeriesDetails() {
    ...
    const [heartbeatSeries, setHeartbeatSeries] = useState<HeartbeatSeriesSample | null>(null);
    ...
    if (!heartbeatSeries) {
        return <List><ListItem title="Loading heartbeat series details..." /></List>;
    }
    ...
    return (
        <List scrollEnabled>
            ...
            {heartbeatSeries.device ? <Section title='Device'>
                {heartbeatSeries.device.name ? <ListItem
                    title='Name'
                    subtitle={heartbeatSeries.device.name} /> : null}
                {heartbeatSeries.device.model ? <ListItem
                    title='Model'
                    subtitle={heartbeatSeries.device.model} /> : null}
                {heartbeatSeries.device.manufacturer ? <ListItem
                    title='Manufacturer'
                    subtitle={heartbeatSeries.device.manufacturer} /> : null}
                {heartbeatSeries.device.hardwareVersion ? <ListItem
                    title='Hardware Version'
                    subtitle={heartbeatSeries.device.hardwareVersion} /> : null}
                {heartbeatSeries.device.softwareVersion ? <ListItem
                    title='Software Version'
                    subtitle={heartbeatSeries.device.softwareVersion} /> : null}
            </Section> : null}

            {heartbeatSeries.metadata && Object.keys(heartbeatSeries.metadata).length > 0 && <Section title='Metadata'>
            ...
            {heartbeatSeries.sourceRevision && <Section title='Source'>
                {heartbeatSeries.sourceRevision.source?.name && <ListItem
                    title='Source Name'
                    subtitle={heartbeatSeries.sourceRevision.source.name} />}
                {heartbeatSeries.sourceRevision.source?.bundleIdentifier && <ListItem
                    title='Bundle Identifier'
                    subtitle={heartbeatSeries.sourceRevision.source.bundleIdentifier} />}
                {heartbeatSeries.sourceRevision.version && <ListItem
                    title='Version'
                    subtitle={heartbeatSeries.sourceRevision.version} />}
                {heartbeatSeries.sourceRevision.operatingSystemVersion && <ListItem
                    title='OS Version'
