...
export const QueryInfo = ({ 
    ...
    deletedSamples,
    onFetchMore
}: { onFetchMore?: () => void, queryTime?: number, anchor?: string, deletedSamples?: readonly string[] }) => {
    return (
        <Host style={{ padding: 16 }}>
            <VStack padding={16} spacing={8}>
                <Text>{queryTime ? `Query time: ${queryTime}ms` : 'Loading...'}</Text>
                {deletedSamples && deletedSamples.length > 0 ? <Text>{`Deleted samples: ${deletedSamples?.length}`}</Text> : null}
